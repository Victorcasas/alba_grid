...
public class ReportInputParser extends DefaultInputServiceParser {
	
	private static GridLog logger = GridLogFactory.getInstance().getLog(ReportInputParser.class);
	...
	public Request parse(Object[] inputs) throws TaskcontrollerParserException {
		...
		for (Object obj : inputs) {
			...
			if (obj instanceof OMElement) {
				...
				OMElement params = ome.getFirstChildWithName(new QName("", "reportParams"));
				extractParamToReq(params,"initDate",req);
				extractParamToReq(params,"endDate",req);
				extractParamToReq(params,"op",req);
				extractParamToReq(params,"output",req);
				extractParamToReq(params,"jclDemanded",req);
			} else {
	...
	public OMElement responseParse(Response input) throws TaskControllerException {	
		...
		gridJobResponseElto.addAttribute("reportReference",(String)input.get("mdsReference"),ns);
