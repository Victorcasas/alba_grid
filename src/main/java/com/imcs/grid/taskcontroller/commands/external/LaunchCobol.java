...
public class LaunchCobol extends TaskControllerCommand {
	...
	public Forward execute(Action action, Properties parameters, JobState status) throws TaskControllerException {
		...
		try {
			logger.info("******************************************************");
			logger.info("[COBOL] Init execute cobol with pid " + pid);
			...
			ProcessConnectorBinding connector = new ProcessConnectorBinding(cobolWorkingDir, "rexx Cobol_Pipe", 
																			pcParameters, pid);
			...
			logger.info("******************************************************");
			logger.info("[COBOL] End execute cobol with pid :: " + pid);
			...
			if (exitStatusCode.equalsIgnoreCase("KO")) {
				...
				if ((mode != null) && (mode.equalsIgnoreCase("parallel"))){
					try{
						TaskControllerToBrokerClient.callAddResultParallel(jcl,step,"","","Error - Launch Cobol","","","");
					} catch (AxisFault e) {
						logger.error("Error adding result parallel :: " + e);
					} catch (InterruptedException e) {
						logger.error("Error adding result parallel :: " + e);
					}
//					TaskControllerEventEngine.getInstance().callAddResultParallel(jcl, step, "", "", "Error - Launch Cobol");
				}
				...
				if ((mode != null) && (mode.equalsIgnoreCase("parallel"))){
					try{
						TaskControllerToBrokerClient.callAddResultParallel(jcl,step,"","","ERROR executing cobol","","","");
					} catch (AxisFault e) {
						logger.error("Error adding result parallel :: " + e);
					} catch (InterruptedException e) {
						logger.error("Error adding result parallel :: " + e);
...
//					TaskControllerEventEngine.getInstance().callAddResultParallel(jcl, step, "", "","ERROR executing cobol");
...
//				TaskControllerEventEngine.getInstance().callAddResultParallel(jcl, step, "", "", "ERROR executing cobol");
			...
			action.getResponse().put("gridrcDesc","Error executing Cobol service");
			return action.findForward("error");
		}
	}
